...
- 🎯 Entry points to execute your program using `python -m <home_security>` or `$ home_security` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/asrulsibaoel/home-security/branch/main/graph/badge.svg?token=home-security_token_here)](https://codecov.io/gh/asrulsibaoel/home-security)
[![CI](https://github.com/asrulsibaoel/home-security/actions/workflows/main.yml/badge.svg)](https://github.com/asrulsibaoel/home-security/actions/workflows/main.yml)

Awesome home_security created by asrulsibaoel

## Install

from source
```bash
git clone https://github.com/asrulsibaoel/home-security home_security
cd home_security
make install
```
...
```bash
pip install home_security
```

## Executing

```bash
$ home_security run --port 8080
```

or

```bash
python -m home_security run --port 8080
```

or

```bash
$ uvicorn home_security:app
```
...
```bash
❯ home_security --help
Usage: home_security [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ home_security create-user --help
Usage: home_security create-user [OPTIONS] USERNAME PASSWORD
...
```bash
home_security create-user admin admin --superuser
```
...
Run with `home_security run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `home_security create-user`.
...
-----------------------------------------------------
home_security/__init__.py              4      0   100%
home_security/app.py                  16      1    94%
home_security/cli.py                  21      0   100%
home_security/config.py                5      0   100%
home_security/db.py                   10      0   100%
home_security/models/__init__.py       0      0   100%
home_security/models/content.py       47      1    98%
home_security/routes/__init__.py      11      0   100%
home_security/routes/content.py       52     25    52%
home_security/routes/security.py      15      1    93%
home_security/routes/user.py          52     26    50%
home_security/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from home_security.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in home_security/default.toml.

### As environment variables
```bash
export home_security_KEY=value
export home_security_KEY="@int 42"
export home_security_KEY="@jinja {{ this.db.uri }}"
export home_security_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
home_security_ENV=production home_security run
```
